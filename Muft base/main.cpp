...
void vac_bypass(bool switcher) // thx for http://github.com/danielkrupinski/VAC-Bypass
{
	if (switcher){
		HKEY key = NULL;
		std::cout << "" << std::endl;
		std::cout << " [Muft.beta] Bem vindo ao Muft.beta" << std::endl;
		Sleep(3000);
		if (!RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\Valve\\Steam", 0, KEY_QUERY_VALUE, &key)) {
			...
			if (!RegQueryValueExW(key, L"SteamExe", NULL, NULL, (LPBYTE)(steamPath + 1), &steamPathSize)) {
				...
				std::cout << " [Muft.beta] fechando steam process // vac bypass Funcionando" << std::endl;
				Sleep(3000);
				...
				if (CreateProcessW(NULL, steamPath, NULL, NULL, FALSE, 0, NULL, NULL, &info, &processInfo)) {
					...
					std::cout << " [Muft.beta] carregando as offsets" << std::endl;
					Sleep(500);
					std::cout << " [Muft.beta] abra a steam..." << std::endl;
					Sleep(1000);
					...
					std::cout << " [Muft.beta] injetado" << std::endl;
					Sleep(3000);
	...
	std::cout << " [Muft.beta] abra o csgo" << std::endl;
}
